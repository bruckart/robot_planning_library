...
DroidEndItem::DroidEndItem(int x, int y, double r)
:
...
double DroidEndItem::radius() const
{
    return m_radius;
}

QRectF DroidEndItem::boundingRect() const
{
    return QRectF(m_x, m_y, m_radius*2, m_radius*2);
}

QPainterPath DroidEndItem::shape() const
{
    QPainterPath path;
...
void DroidEndItem::paint(QPainter* painter, 
...
// void DeathStarItem::keyPressEvent(QKeyEvent* event)
// {
//     std::clog << "keyPressEvent()" << std::endl;
